...
- [~] 4.5 Read S26 Ultra soc_id/dsp_arch on-device and generate the v81 context binary — DEVICE INFO READ: S26 Ultra = SM-S948W, SoC SM8850 (Snapdragon 8 Elite Gen 5), platform=canoe. BLOCKED on SDK version: QAIRT 2.26.2 only ships Hexagon v66..v75, but SM8850's HTP is newer (v79+), so the context binary can't be built here. Needs QAIRT >= 2.47 (Software Center). App's DeviceInfo also returns htp=null for SM8850 (needs the arch mapping once known). Until then the S26 runs the ORT/CPU fallback.
