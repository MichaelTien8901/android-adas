...
- [ ] 1.1 Add `Prefs`: `dashcamEnabled` (Bool, default false), `dashcamSegmentMinutes`
      (Int, default 3), `dashcamMaxStorageMb` (Int, default 2048), with keys + getters/setters.
- [ ] 1.2 Add a "Recording" section to `SettingsActivity` + layout: enable toggle and
      segment-minutes / max-storage-MB inputs (steppers or a small picker), with hint strings.
- [ ] 1.3 String resources for the new labels/hints.

## 2. Storage / retention (RecordingStore)

- [ ] 2.1 `RecordingStore`: resolve the clip dir (`getExternalFilesDir("dashcam")`), create it,
      expose the path for retrieval.
- [ ] 2.2 Filename helper: `dashcam_yyyy-MM-dd_HHmmss.mp4` (sortable, monotonic suffix on
      collision) and a parser/sorter (by name datetime, fallback `lastModified`).
- [ ] 2.3 Retention: sum dir size; when over `dashcamMaxStorageMb` (or device free space below
...
- [ ] 3.1 Add CameraX video dependency (`androidx.camera:camera-video`).
- [ ] 3.2 `DashcamRecorder`: build a `Recorder`/`VideoCapture`; start/stop a `Recording` to a
      datetime-named file in the clip dir; expose `isRecording`.
- [ ] 3.3 Segment rollover: at `dashcamSegmentMinutes`, finalize the current `Recording` and
      immediately start the next (background executor timer); call `RecordingStore` retention
      on each finalize and at start.
- [ ] 3.4 Failure isolation: catch encoder/storage errors, surface a status flag, never crash
      the session; if free space can't be secured, stop recording.

## 4. Camera + session wiring

- [ ] 4.1 `CameraController`: optionally bind `VideoCapture` as a 3rd use case (Preview +
...
- [ ] 4.2 `DrivingService`: own the `DashcamRecorder` lifecycle — auto-start when
...
- [ ] 5.1 Add a record on/off button to the driving overlay; wire to start/stop within the
      session (overrides the Settings default for the session).
- [ ] 5.2 Recording-state indicator (e.g. red dot / "REC") reflecting `isRecording`.
